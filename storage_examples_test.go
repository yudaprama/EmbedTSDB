...
func ExampleStorage_InsertRows() {
	...
	err = storage.InsertRows([]embedtsdb.Row{
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000000, Value: 0.1}},
	})
	if err != nil {
		panic(err)
	}
...
// simulates writing and reading concurrently.
	...
	go func() {
		...
		for i := int64(1600000000); i < 1600010000; i++ {
			...
			go func(timestamp int64) {
				defer wg.Done()
				if err := storage.InsertRows([]embedtsdb.Row{
					{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: timestamp}},
				}); err != nil {
					panic(err)
				}
			}(i)
	...
	go func() {
		...
		for i := 0; i < 100; i++ {
			...
			go func() {
				...
				if errors.Is(err, embedtsdb.ErrNoDataPoints) {
					return
				}
				if err != nil {
					panic(err)
...
func ExampleStorage_Select_from_memory() {
	tmpDir, err := os.MkdirTemp("", "embedtsdb-example")
	if err != nil {
		panic(err)
	}
	...
	storage, err := embedtsdb.NewStorage(
		embedtsdb.WithDataPath(tmpDir),
		embedtsdb.WithPartitionDuration(2*time.Hour),
		embedtsdb.WithTimestampPrecision(embedtsdb.Seconds),
	)
	if err != nil {
		panic(err)
	...
	for timestamp := int64(1600000000); timestamp < 1600000050; timestamp++ {
		err := storage.InsertRows([]embedtsdb.Row{
			{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: timestamp, Value: 0.1}},
		})
		if err != nil {
			panic(err)
		}
	...
	for timestamp := int64(1600000050); timestamp < 1600000100; timestamp++ {
		err := storage.InsertRows([]embedtsdb.Row{
			{Metric: "metric2", DataPoint: embedtsdb.DataPoint{Timestamp: timestamp, Value: 0.2}},
		})
		if err != nil {
			panic(err)
		}
...
func ExampleStorage_Select_from_disk() {
	...
	storage, err := embedtsdb.NewStorage(
		embedtsdb.WithDataPath(tmpDir),
		embedtsdb.WithPartitionDuration(100*time.Second),
		embedtsdb.WithTimestampPrecision(embedtsdb.Seconds),
	)
	if err != nil {
		panic(err)
	...
	storage, err = embedtsdb.NewStorage(
		embedtsdb.WithDataPath(tmpDir),
		embedtsdb.WithPartitionDuration(10*time.Second),
		embedtsdb.WithTimestampPrecision(embedtsdb.Seconds),
	)
	if err != nil {
		panic(err)
...
func ExampleStorage_Select_from_memory_out_of_order() {
	...
	err = storage.InsertRows([]embedtsdb.Row{
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000000, Value: 0.1}},
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000002, Value: 0.1}},
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000001, Value: 0.1}},
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000003, Value: 0.1}},
	})
	if err != nil {
		panic(err)
	}
...
func ExampleStorage_InsertRows_outdated() {
	...
	storage, err := embedtsdb.NewStorage(
		embedtsdb.WithDataPath(tmpDir),
		embedtsdb.WithTimestampPrecision(embedtsdb.Seconds),
		embedtsdb.WithPartitionDuration(3*time.Second),
	)
	if err != nil {
		panic(err)
	}
	...
	err = storage.InsertRows([]embedtsdb.Row{
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000001, Value: 0.1}, Metric: "metric1"},
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000003, Value: 0.1}, Metric: "metric1"},
	})
	if err != nil {
		panic(err)
	}
	err = storage.InsertRows([]embedtsdb.Row{
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000004, Value: 0.1}, Metric: "metric1"},
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000005, Value: 0.1}, Metric: "metric1"},
	})
	if err != nil {
		panic(err)
	}
	...
	err = storage.InsertRows([]embedtsdb.Row{
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000002, Value: 0.1}, Metric: "metric1"},
	})
	if err != nil {
		panic(err)
	}
...
func ExampleStorage_InsertRows_expired() {
	...
	err = storage.InsertRows([]embedtsdb.Row{
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000004, Value: 0.1}, Metric: "metric1"},
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000005, Value: 0.1}, Metric: "metric1"},
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000006, Value: 0.1}, Metric: "metric1"},
	})
	if err != nil {
		panic(err)
	}
	err = storage.InsertRows([]embedtsdb.Row{
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000007, Value: 0.1}, Metric: "metric1"},
		{DataPoint: embedtsdb.DataPoint{Timestamp: 1600000008, Value: 0.1}, Metric: "metric1"},
	})
	if err != nil {
		panic(err)
	}
...
func ExampleStorage_InsertRows_concurrent() {
	...
	if err := storage.InsertRows([]embedtsdb.Row{
		{Metric: "metric1", DataPoint: embedtsdb.DataPoint{Timestamp: 1600000000}},
	}); err != nil {
		panic(err)
	}
