...
// Object pools for encoder/decoder reuse
var (
	encoderPool = sync.Pool{
		New: func() interface{} {
			return &gorillaEncoder{
				buf: &bstream{stream: make([]byte, 0, 1024)}, // Pre-allocate buffer
			}
		},
	}
	...
	bstreamPool = sync.Pool{
		New: func() interface{} {
			return &bstream{stream: make([]byte, 0, 1024)}
		},
	}
)
...
func newSeriesEncoder(w io.Writer) seriesEncoder {
	encoder := encoderPool.Get().(*gorillaEncoder)
	encoder.w = w
	encoder.resetEncoder()
	return encoder
}

func putSeriesEncoder(encoder seriesEncoder) {
	if gorilla, ok := encoder.(*gorillaEncoder); ok {
		encoderPool.Put(gorilla)
	}
}
...
func (e *gorillaEncoder) resetEncoder() {
	if e.buf == nil {
		e.buf = bstreamPool.Get().(*bstream)
	}
	e.buf.reset()
	e.t0 = 0
	e.t1 = 0
...
func newSeriesDecoder(r io.Reader) (seriesDecoder, error) {
	...
	decoder := decoderPool.Get().(*gorillaDecoder)
	decoder.resetDecoder()
	decoder.br = newBReader(b)
	return decoder, nil
}

func putSeriesDecoder(decoder seriesDecoder) {
	if gorilla, ok := decoder.(*gorillaDecoder); ok {
		decoderPool.Put(gorilla)
	}
}
...
func (d *gorillaDecoder) resetDecoder() {
	d.numRead = 0
	d.t = 0
	d.tDelta = 0
	d.v = 0
